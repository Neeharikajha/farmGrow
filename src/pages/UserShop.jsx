...
export default function UserShop() {
  ...
  const [filters, setFilters] = useState({ category: "All", organic: "Any", min: "", max: "", rating: 0 });
  ...
  const categories = useMemo(() => ["All", ...Array.from(new Set(RAW_VEG.map((v) => v.category)))], []);
  
  const filtered = useMemo(() => {
    const min = parseFloat(filters.min);
    const max = parseFloat(filters.max);
    return RAW_VEG.filter((v) => {
      const catOk = (category === "All" || v.category === category) && (filters.category === "All" || v.category === filters.category);
      const orgOk = filters.organic === "Any" ? true : filters.organic === "Yes" ? v.isOrganic : !v.isOrganic;
      const priceOk = (isNaN(min) ? true : v.pricePerKg >= min) && (isNaN(max) ? true : v.pricePerKg <= max);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Navbar cartCount={cart.reduce((acc, item) => acc + item.qty, 0)} toggleCart={() => {}} />
